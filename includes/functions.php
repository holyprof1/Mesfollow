	...
	*/
	...
	public function iN_Sen($mycd, $mycdStatus,$base_url){
		...
		if($check && $mycdStatus == '1'){
		   ...
		   return header('Location:' . $base_url . base64_decode('YmVsZWdhbD9zdGVwPXJlcXVpcmVtZW50cw=='));
		}
	}
	/**
...
// REPLACE the iN_UserCover function in includes/functions.php (around line 240)
// with this COMPLETE function:

// REPLACE the iN_UserCover function in includes/functions.php (around line 240)
// with this COMPLETE function:

// REPLACE iN_UserCover function in includes/functions.php (line ~240)
// This version handles BOTH cover_id (old) and direct path (new)
...
public function iN_UserCover($uid, $base_url) {
    ...
    $oceanStatus = isset($s3['ocean_status']) ? $s3['ocean_status'] : '0';
    $userGender = isset($row['user_gender']) ? $row['user_gender'] : NULL;
    $rowCover = isset($row['user_cover']) ? trim($row['user_cover']) : NULL;
    
    $coverPath = NULL;
    
    // FIX: Check if user_cover is a number (cover_id) or a path
    if (!empty($rowCover)) {
        if (ctype_digit($rowCover)) {
            // OLD SYSTEM: user_cover is a cover_id, look it up
            $coverPath = $this->iN_GetUploadedCoverURL($uid, $rowCover);
            
            // If cover_id doesn't exist in i_user_covers, try to get latest cover for this user
            if (empty($coverPath) || $coverPath === false || $coverPath === NULL) {
                $fallbackQuery = mysqli_query($this->db, "SELECT cover_path FROM i_user_covers WHERE iuid_fk='{$uid}' ORDER BY cover_id DESC LIMIT 1");
                if ($fallbackQuery && $fallbackRow = mysqli_fetch_assoc($fallbackQuery)) {
                    $coverPath = isset($fallbackRow['cover_path']) ? $fallbackRow['cover_path'] : NULL;
                }
            }
        } else {
            // NEW SYSTEM: user_cover is already a path
            $coverPath = $rowCover;
        }
    }
    
    // Sanitize: Remove obvious bogus values
    if ($coverPath === '/' || $coverPath === '#' || $coverPath === '0' || 
        $coverPath === $base_url || $coverPath === rtrim($base_url, '/') || 
        $coverPath === rtrim($base_url, '/') . '/') {
        $coverPath = NULL;
    }
    
    // If we have a valid cover path, build the full URL
    if (!empty($coverPath) && $coverPath !== false && $coverPath !== NULL) {
        if ($s3Status == 1) {
        ...
        // Return default cover based on gender
        if ($userGender == 'male') {
...
/**
 * Count how many times a post has been saved.
 * Works with PDO or mysqli.
 */
public function iN_CountSavedPosts($postID) {
    $postID = (int)$postID;

    try {
        // PDO
        if ($this->db instanceof PDO) {
            $stmt = $this->db->prepare("SELECT COUNT(*) AS total FROM i_saved_posts WHERE saved_post_id = ?");
            $stmt->execute([$postID]);
...
/*Insert Notification For Subscription Expiring Soon*/
	public function iN_InsertNotificationForSubscriptionExpiring($subscriberID, $creatorID, $daysRemaining) {
		$subscriberID = mysqli_real_escape_string($this->db, $subscriberID);
		$creatorID = mysqli_real_escape_string($this->db, $creatorID);
		$time = time();
		$notType = 'subscription_expiring_' . $daysRemaining . 'd';
		mysqli_query($this->db, "INSERT INTO i_user_notifications(not_iuid, not_own_iuid, not_not_type, not_time)VALUES('$creatorID','$subscriberID','$notType','$time')") or die(mysqli_error($this->db));
		return true;
	}

	/*Insert Notification For Subscription Expired*/
	public function iN_InsertNotificationForSubscriptionExpired($subscriberID, $creatorID) {
		$subscriberID = mysqli_real_escape_string($this->db, $subscriberID);
		$creatorID = mysqli_real_escape_string($this->db, $creatorID);
		$time = time();
		mysqli_query($this->db, "INSERT INTO i_user_notifications(not_iuid, not_own_iuid, not_not_type, not_time)VALUES('$creatorID','$subscriberID','subscription_expired','$time')") or die(mysqli_error($this->db));
		return true;
	}

	/*Check And Process Expired Subscriptions*/
	public function iN_ProcessExpiredSubscriptions() {
		// Get all active subscriptions that have passed their end date
		$query = mysqli_query($this->db, "SELECT subscription_id, iuid_fk, subscribed_iuid_fk FROM i_user_subscriptions WHERE status = 'active' AND UNIX_TIMESTAMP(plan_period_end) < UNIX_TIMESTAMP(NOW())") or die(mysqli_error($this->db));
		
		$expiredCount = 0;
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			$subscriptionID = $row['subscription_id'];
			$subscriberID = $row['iuid_fk'];
			$creatorID = $row['subscribed_iuid_fk'];
			...
			// Update friend status from subscriber to follower
			mysqli_query($this->db, "UPDATE i_friends SET fr_status = 'flwr' WHERE fr_one = '$subscriberID' AND fr_two = '$creatorID' AND fr_status = 'subscriber'") or die(mysqli_error($this->db));
			
			// Send expiration notification
			$this->iN_InsertNotificationForSubscriptionExpired($subscriberID, $creatorID);
...
/*Send Notifications For Expiring Subscriptions*/
	public function iN_SendExpiringSubscriptionNotifications() {
		$notificationsSent = 0;
		$daysToCheck = [5, 2, 0]; // 5 days, 2 days, same day
		
		foreach ($daysToCheck as $days) {
			// Calculate target date range
			if ($days == 0) {
				// For same day: check if expires within next 24 hours
				$targetDateStart = date('Y-m-d H:i:s');
				$targetDateEnd = date('Y-m-d 23:59:59', strtotime('+1 day'));
			} else {
				// For 2 days or 5 days: check if expires on that specific day
				$targetDateStart = date('Y-m-d 00:00:00', strtotime("+$days days"));
				$targetDateEnd = date('Y-m-d 23:59:59', strtotime("+$days days"));
			}
			
			// Find subscriptions expiring in target range
			$query = mysqli_query($this->db, "
				SELECT s.subscription_id, s.iuid_fk, s.subscribed_iuid_fk, s.plan_period_end
				FROM i_user_subscriptions s
				WHERE s.status = 'active' 
				AND s.plan_period_end BETWEEN '$targetDateStart' AND '$targetDateEnd'
			") or die(mysqli_error($this->db));
			...
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				...
				$checkDuplicate = mysqli_query($this->db, "
					SELECT not_id FROM i_user_notifications 
					WHERE not_own_iuid = '$subscriberID' 
					AND not_iuid = '$creatorID' 
					AND not_not_type = 'subscription_expiring_{$days}d'
					AND not_time > UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 12 HOUR))
					LIMIT 1
				") or die(mysqli_error($this->db));
				
				if (mysqli_num_rows($checkDuplicate) == 0) {
					$this->iN_InsertNotificationForSubscriptionExpiring($subscriberID, $creatorID, $days);
					$notificationsSent++;
				}
			}
		}
	...
	/*Check user is in FLWR*/
	/*Check user is in Subscriber - WITH EXPIRATION CHECK*/
	/*Check user is in Subscriber - WITH EXPIRATION CHECK*/
public function iN_CheckUserIsInSubscriber($userID, $uID) {
	$userID = mysqli_real_escape_string($this->db, $userID);
	$uID = mysqli_real_escape_string($this->db, $uID);
	
	// First check if they have subscriber status in friends table
	$GetStatus = @mysqli_query($this->db, "SELECT fr_status FROM i_friends WHERE fr_one='$userID' AND fr_two='$uID' AND fr_status = 'subscriber'");
	
	if (!$GetStatus) {
		// Query failed, return false safely
		return false;
	}
	
	if (mysqli_num_rows($GetStatus) == 1) {
		// Now verify the subscription is still active and hasn't expired
		$checkExpired = @mysqli_query($this->db, "SELECT subscription_id FROM i_user_subscriptions WHERE iuid_fk='$userID' AND subscribed_iuid_fk='$uID' AND status='active' AND UNIX_TIMESTAMP(plan_period_end) >= UNIX_TIMESTAMP(NOW()) ORDER BY subscription_id DESC LIMIT 1");
		...
		if (mysqli_num_rows($checkExpired) == 1) {
			return true;
		} else {
			// Subscription expired - update it silently without dying
			@mysqli_query($this->db, "UPDATE i_user_subscriptions SET status = 'inactive', in_status = '1', finished = '1' WHERE iuid_fk='$userID' AND subscribed_iuid_fk='$uID' AND status='active' AND UNIX_TIMESTAMP(plan_period_end) < UNIX_TIMESTAMP(NOW())");
			@mysqli_query($this->db, "UPDATE i_friends SET fr_status = 'flwr' WHERE fr_one = '$userID' AND fr_two='$uID' AND fr_status = 'subscriber'");
			return false;
		}
	} else {
		return false;
...
public function iN_ChatUserList($userID, $limit) {
    ...
    $limit     = (int)$userLimit ?: 24;

    $moreUser = '';
    if (!empty($lastUID)) {
        $moreUser = " AND iu.iuid < '{$lastUID}' ";
    }

    $sql = "
        SELECT iu.*
        FROM i_users iu FORCE INDEX(ixForceUser)
        WHERE iu.uStatus IN('1','3')
          AND iu.profile_category = '{$requested}'
          AND iu.profile_category <> '' AND iu.profile_category IS NOT NULL
          {$moreUser}
        ORDER BY iu.iuid DESC
        LIMIT {$limit}
    ";

    $data = [];
    $q = mysqli_query($this->db, $sql) or die(mysqli_error($this->db));
    while ($row = mysqli_fetch_array($q, MYSQLI_ASSOC)) { $data[] = $row; }
    return !empty($data) ? $data : null;
}
	...
	public function iN_SavedPosts($userID, $lastPostID, $showingPost) {
		...
		INNER JOIN i_users U FORCE INDEX (ixForceUser)
			ON S.iuid_fk = U.iuid AND U.uStatus IN('1','3')
		WHERE S.iuid_fk = '$userID' $morePost ORDER BY S.saved_post_id DESC LIMIT $showingPosts") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($getData, MYSQLI_ASSOC)) {
			$data[] = $row;
		}
		if (!empty($data)) {
...
public function iN_UpdateVideoCost($userID, $videoCost) {
    if ($this->iN_CheckUserExist($userID) == 1) {
        $v = (int)$videoCost;
        if ($v < 0) { $v = 0; }
        // Extra safety: cap again here using the global constant
        if (defined('MF_MAX_VIDEO_CALL_PRICE_POINTS') && MF_MAX_VIDEO_CALL_PRICE_POINTS > 0 && $v > MF_MAX_VIDEO_CALL_PRICE_POINTS) {
            $v = (int)MF_MAX_VIDEO_CALL_PRICE_POINTS;
        }
        mysqli_query($this->db, "UPDATE i_users SET video_call_price = '$v' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
        return true;
    } else {
        return false;
    }
...
public function iN_GetTotalProfileViewsByDateRange($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    
    $query = mysqli_query($this->db, "
        SELECT COUNT(DISTINCT pv.id) AS totalViews
        FROM i_post_views pv
        INNER JOIN i_posts p ON pv.post_id = p.post_id
        WHERE p.post_owner_id = '$userID'
        AND UNIX_TIMESTAMP(pv.view_time) BETWEEN '$startTime' AND '$endTime'
    ") or die(mysqli_error($this->db));
    
    $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
    return isset($row['totalViews']) ? $row['totalViews'] : '0';
}

/**
 * Get daily profile views breakdown for charts
 */
public function iN_GetDailyProfileViews($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        SELECT 
            DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(pv.view_time))) as view_date,
            COUNT(DISTINCT pv.id) AS daily_views
        FROM i_post_views pv
        INNER JOIN i_posts p ON pv.post_id = p.post_id
        WHERE p.post_owner_id = '$userID'
        AND UNIX_TIMESTAMP(pv.view_time) BETWEEN '$startTime' AND '$endTime'
...
/**
 * Get followers count in a date range
 */
public function iN_GetFollowersByDateRange($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        ...
        WHERE fr_two = '$userID'
        AND fr_status = 'flwr'
        AND fr_time BETWEEN '$startTime' AND '$endTime'
    ") or die(mysqli_error($this->db));
    
    $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
    return isset($row['totalFollowers']) ? $row['totalFollowers'] : '0';
}

/**
 * Get daily followers breakdown for charts
 */
public function iN_GetDailyFollowers($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        SELECT 
            DATE(FROM_UNIXTIME(fr_time)) as follow_date,
            COUNT(*) AS daily_followers
        FROM i_friends
        WHERE fr_two = '$userID'
        AND fr_status = 'flwr'
        AND fr_time BETWEEN '$startTime' AND '$endTime'
...
/**
 * Get total likes on user's posts in a date range
 */
public function iN_GetTotalLikesByDateRange($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        ...
        FROM i_post_likes pl
        INNER JOIN i_posts p ON pl.post_id_fk = p.post_id
        WHERE p.post_owner_id = '$userID'
        AND pl.like_time BETWEEN '$startTime' AND '$endTime'
    ") or die(mysqli_error($this->db));
    
    $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
    return isset($row['totalLikes']) ? $row['totalLikes'] : '0';
}

/**
 * Get daily likes breakdown for charts
 */
public function iN_GetDailyLikes($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        SELECT 
            DATE(FROM_UNIXTIME(pl.like_time)) as like_date,
...
/**
 * Get total comments on user's posts in a date range
 */
public function iN_GetTotalCommentsByDateRange($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        ...
        FROM i_post_comments pc
        INNER JOIN i_posts p ON pc.comment_post_id_fk = p.post_id
        WHERE p.post_owner_id = '$userID'
        AND pc.comment_time BETWEEN '$startTime' AND '$endTime'
    ") or die(mysqli_error($this->db));
    
    $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
    return isset($row['totalComments']) ? $row['totalComments'] : '0';
}

/**
 * Get daily comments breakdown for charts
 */
public function iN_GetDailyComments($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        SELECT 
            DATE(FROM_UNIXTIME(pc.comment_time)) as comment_date,
...
/**
 * Get new subscribers in a date range
 */
public function iN_GetSubscribersByDateRange($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        ...
        WHERE subscribed_iuid_fk = '$userID'
        AND status = 'active'
        AND UNIX_TIMESTAMP(created) BETWEEN '$startTime' AND '$endTime'
    ") or die(mysqli_error($this->db));
...
/**
 * Get daily subscribers breakdown for charts
 */
public function iN_GetDailySubscribers($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
 ...
 */
public function iN_GetPostsByDateRange($userID, $startTime, $endTime) {
    $userID = mysqli_real_escape_string($this->db, $userID);
    $startTime = mysqli_real_escape_string($this->db, $startTime);
    $endTime = mysqli_real_escape_string($this->db, $endTime);
    ...
    $query = mysqli_query($this->db, "
        ...
        AND post_created_time BETWEEN '$startTime' AND '$endTime'
    ") or die(mysqli_error($this->db));
    
    $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
    return isset($row['totalPosts']) ? $row['totalPosts'] : '0';
}

/**
 * Calculate engagement rate (likes + comments / views * 100)
 */
public function iN_CalculateEngagementRate($views, $likes, $comments) {
    if ($views == 0) return 0;
 ...
 */
public function iN_CalculateGrowthPercentage($current, $previous) {
    if ($previous == 0) {
        if ($current > 0) {
...
} // <-- This CLOSES the iN_UPDATES class

/**
 * Record post view - ONE VIEW PER USER/IP
 */
function recordPostView($postId, $db, $logedIn, $userID) {
    $postId = (int)$postId;
    if (empty($postId) || !$db) return false;
    
    // Get visitor info
    $viewerUserId = ($logedIn == 1 && $userID) ? (int)$userID : null;
    $viewerIp = $_SERVER['REMOTE_ADDR'] ?? '';
    $viewerIp = filter_var($viewerIp, FILTER_VALIDATE_IP) ? $viewerIp : '';
    
    if (!$viewerUserId && !$viewerIp) return false;
    
    // Check if already viewed
    if ($viewerUserId) {
        $stmt = $db->prepare("SELECT COUNT(*) as c FROM i_post_views WHERE post_id = ? AND viewer_user_id = ?");
        if (!$stmt) return false;
        $stmt->bind_param("ii", $postId, $viewerUserId);
    } else {
        $stmt = $db->prepare("SELECT COUNT(*) as c FROM i_post_views WHERE post_id = ? AND viewer_ip = ? AND viewer_user_id IS NULL");
        if (!$stmt) return false;
        $stmt->bind_param("is", $postId, $viewerIp);
    }
    ...
    // Record new view
    $stmt = $db->prepare("INSERT INTO i_post_views (post_id, viewer_user_id, viewer_ip) VALUES (?, ?, ?)");
    if (!$stmt) return false;
    $stmt->bind_param("iis", $postId, $viewerUserId, $viewerIp);
