...
class PostsHelperServiceProvider extends ServiceProvider
{
    ...
    public static function getPostComments($post_id, $limit = 9, $order = 'DESC', $encodePostsToHtml = false)
    {
        ...
        if ($encodePostsToHtml) {
            ...
            $commentsData = $comments->map(function ($comment, $index) {
                $post = ['id' => $comment->id, 'post_id' => $comment->post->id, 'html' => View::make('elements.feed.post-comment')->with('comment', $comment)->with('isFirst', $index === 0)->render()];
                return $post;
            });
