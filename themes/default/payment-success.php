    ...
    <div class="wrapper">
           <?php include("layouts/left_menu.php"); ?>
              <div class="th_middle">
				 ...
				 <div class="payment_actions" style="margin-top:18px; display:flex; justify-content:center;">
  <a
    href="/"
    class="i_button btn btn-primary"
    style="display:inline-block; padding:10px 18px; border-radius:8px; text-decoration:none;"
    onclick="event.preventDefault(); try { if (document.referrer && window.history.length > 1) { history.back(); } else { window.location.href = '/'; } } catch(e) { window.location.href = '/'; }"
  >
    <?php echo iN_HelpSecure($LANG['go_back']); ?>
  </a>
</div>
