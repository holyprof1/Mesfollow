<?php
/* Points subscription popup (Monthly + Yearly) – uses .bcmSubsPoint.
 * This DOES NOT touch Paystack; it follows your existing points flow.
 */
?>
<div class="i_modal_bg_in" role="dialog" aria-modal="true" aria-labelledby="subscriptionPointModalTitle">
  <div class="i_modal_in_in i_sf_box">
    <div class="i_modal_content">
      ...
      <div class="i_f_other" id="pr_u_id">
        <div class="i_u_name" id="subscriptionPointModalTitle">
        ...
        <div class="support_not"><?php echo iN_HelpSecure(preg_replace('/{.*?}/', $f_userfullname, $LANG['subscribeNot']));?></div>
        ...
        if ($rows = $iN->iN_UserSusbscriptionOffers($f_userID)) {
          foreach ($rows as $u) {
            if ($u['plan_type'] === 'monthly' || $u['plan_type'] === 'yearly') {
              $offers[] = ['plan_id'=>$u['plan_id'],'amount'=>$u['amount'],'plan_type'=>$u['plan_type']];
            }
          }
        }
        if (empty($offers)) {
          ...
          $q = @mysqli_query($db,"SELECT IFNULL(sub_month_status,0) mS, IFNULL(sub_month_amount,'') mA,
        ...
        if (!empty($offers)) {
          ...
          foreach ($offers as $o) { ?>
            <div class="i_prices_subscribe">
              <div
                class="subscribe_price_btn bcmSubsPoint"
                id="<?php echo iN_HelpSecure($o['plan_id']); ?>"
                data-u="<?php echo iN_HelpSecure($f_userID); ?>"
                data-amount="<?php echo iN_HelpSecure($o['amount']); ?>"
                data-interval="<?php echo iN_HelpSecure($o['plan_type']); ?>"
              >
                <?php echo html_entity_decode($iN->iN_SelectedMenuIcon('51')); ?>
                <?php echo iN_HelpSecure(preg_replace('/{.*?}/', $o['amount'], $LANG['subscribe_for_point'])); ?>
                / <?php echo iN_HelpSecure($LANG[$o['plan_type']] ?? $o['plan_type']); ?>
              </div>
            </div>
          <?php }
        } else { ?>
